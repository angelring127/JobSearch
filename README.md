...
이 Mac에서 운영 크롤러를 6시간마다 실행하려면 다음 설치기를 실행합니다. 설치기는 운영 `DATABASE_URL`을 macOS 로그인 키체인에서 읽으므로 DB 비밀번호를 저장소나 평문 설정 파일에 복사하지 않습니다. Mac이 켜져 있고 네트워크와 로컬 Codex Bridge를 사용할 수 있어야 합니다.
